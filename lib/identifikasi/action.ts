...
const IdentifikasiSchema = z.object({
    ...
    evidences: z.array(z.object({
        file: z.instanceof(File)
            .refine((file) => file.size > 0, {message: "File is required", path:["file"]})
            .refine((file) => file.size === 0 || file.type.startsWith ("image/"), {
                message: "Only images are allowed", path:["file"]
            })
            .refine((file) => file.size < 8000000, {
                message: "File must less than 8MB", path:["file"]
            }),
    ...
    fotoudara: z.instanceof(File)
        .refine((file) => file.size > 0, {message: "File is required"})
        .refine((file) => file.size === 0 || ["image/", "application/pdf",
            "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
            "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",]
            .some((type) => file.type.startsWith(type)), {
                message: "Unsupported file format. Ensure your file is in the correct format and try again."
            })
        .refine((file) => file.size < 8000000, {
...
export const saveIdentifikasi = async (prevState: any, formData: FormData) => {
    ...
    if (mainFile && mainNamaPemilik && mainBidangLahan) {
        evidences.push({ file: mainFile, namaPemilik: mainNamaPemilik, bidangLahan: mainBidangLahan });
    }
    ...
    formData.forEach((value, key) => {
        if (key.startsWith("file-")) {
            ...
            const bidangLahan = formData.get(`bidangLahan-${index}`);
    ...
    const uniqueEvidences = Array.from(
        new Map(
            evidences.map((evidence) => [`${evidence.file.name}-${evidence.namaPemilik}-${evidence.bidangLahan}`, evidence])
        ).values()
    );
    ...
    const evidenceUrls = await Promise.all(
        evidenceList.map(async (evidence) => {
            ...
            return { file: fileUrl, namaPemilik: evidence.namaPemilik, bidangLahan: evidence.bidangLahan };
        })
    );
...
export const editEvidence = async (id:string, desaId: string, prevState: any, formData: FormData) => {
