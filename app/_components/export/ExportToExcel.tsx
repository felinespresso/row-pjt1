...
const ExportToExcel = ({ apiData, fileName }: any) => {
  ...
  const exportToCSV = (apiData: any, fileName: any) => {
    // Custom header
    const customHeader = [
      { header: "NAMA DESA", key: "namadesa" },
      { header: "SPAN TOWER", key: "spantower" },
      { header: "TANGGAL", key: "tanggal" },
      { header: "HASIL FOTO UDARA", key: "fotoudara" },
    ];

    // Format data sesuai dengan header
    const formattedData = apiData.map((item: any) => ({
      namadesa: item.namadesa || "-",
      spantower: item.spantower || "-",
      tanggal: item.tanggal || "-",
      fotoudara: item.fotoudara || "-",
    }));

    // Membuat worksheet
    const ws = XLSX.utils.json_to_sheet(formattedData, {
      header: customHeader.map((h) => h.key),
    });

    // Menambahkan header ke sheet
    XLSX.utils.sheet_add_aoa(ws, [customHeader.map((h) => h.header)], {
      origin: "A1",
    });

    // Styling header (tebal, warna hijau, teks putih)
    customHeader.forEach((_, index) => {
    ...
    // Styling body (border, align center)
    const bodyStyle = {
      alignment: { horizontal: "center", vertical: "center" },
      border: {
        top: { style: "thin", color: { rgb: "000000" } },
    ...
    // Terapkan styling untuk semua sel di body
    formattedData.forEach((_: any, rowIndex: any) => {
      customHeader.forEach((_, colIndex) => {
        const cell = `${String.fromCharCode(65 + colIndex)}${rowIndex + 2}`; // A2, B2, C2, ...
        if (ws[cell]) {
          ws[cell].s = bodyStyle;
        }
      });
    ...
    // Menyesuaikan lebar kolom berdasarkan panjang teks
    ws["!cols"] = customHeader.map((h) => ({
      wch: Math.max(
        h.header.length + 6,
        ...formattedData.map((row: any) => row[h.key]?.length || 10)
      ),
    }));
