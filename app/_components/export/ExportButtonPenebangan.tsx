// EXPORT BUTTON PENEBANGAN

import { FC } from "react";
import ExcelJS from "exceljs";
import { saveAs } from "file-saver";

interface PenebanganItem {
  namaDesa: string;
  spanTower: string;
  bidangLahan: string;
  namaPemilik: string;
...
interface ExportButtonProps {
  penebanganData: PenebanganItem[];
}

const ExportButtonPenebangan: FC<ExportButtonProps> = ({ penebanganData }) => {
  const exportToExcel = async () => {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet("Tabel Data Penebangan");
    ...
    penebanganData.forEach((item, index) => {
      worksheet.addRow({
        no: index + 1,
    ...
    saveAs(new Blob([buffer]), "Tabel Data Penebangan.xlsx");
  };
...
export default ExportButtonPenebangan;
