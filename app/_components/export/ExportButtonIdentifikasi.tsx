...
interface ExportButtonProps {
  identifikasiData: IdentifikasiItem[];
}

const ExportButtonIdentifikasi: FC<ExportButtonProps> = ({ identifikasiData }) => {
  const exportToExcel = async () => {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet("Tabel Data Identifikasi");
    ...
    identifikasiData.forEach((item, index) => {
      worksheet.addRow({
        no: index + 1,
    ...
    saveAs(new Blob([buffer]), "Tabel Data Identifikasi.xlsx");
  };
...
export default ExportButtonIdentifikasi;
