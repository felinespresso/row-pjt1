...
interface ExportButtonProps {
  musyawarahData: MusyawarahItem[];
}

const ExportButtonMusyawarah: FC<ExportButtonProps> = ({ musyawarahData }) => {
  const exportToExcel = async () => {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet("Tabel Data Musyawarah");
    ...
    musyawarahData.forEach((item, index) => {
      worksheet.addRow({
        no: index + 1,
    ...
    saveAs(new Blob([buffer]), "Tabel Data Musyawarah.xlsx");
  };
...
export default ExportButtonMusyawarah;
