...
const Card = ({ data, itemId, identifikasiId }: { data: Evidences, itemId: string; identifikasiId: string }) => {
        ...
        <>
            <div className="max-w-sm transition-shadow duration-300 bg-white border border-gray-200 rounded-md shadow hover:shadow-lg">
                ...
                <div className="flex items-center justify-between">
                    <EditEvidence id={data.id} itemId={itemId} identifikasiId={identifikasiId} />
                    <DeleteEvidence id={data.id} />
                </div>
            </div>

            {isModalOpen && (
                <div className="fixed inset-0 z-10 flex items-center justify-center bg-black bg-opacity-75 animate-fadeIn"
                    onClick={() => setIsModalOpen(false)}>
                    <div className="relative flex items-center justify-center max-w-[95vw] max-h-[95vh] p-4" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="relative flex items-center justify-center w-auto max-w-full max-h-full">
                            <Image src={data.file} alt={`Evidence ${data.namaPemilik}`} width={750} height={600}
