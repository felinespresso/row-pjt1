...
export default function EvidencePage() {
  ...
  const fetchEvidenceData = async (pageNum: number) => {
    try {
      const response = await fetch(
        `/api/pembayaran/${params.id}/evidence?page=${pageNum}&limit=9`
      );
    ...
    const fetchInitialData = async () => {
      try {
        const [pembayaranResponse, evidenceResponse] = await Promise.all([
          fetch(`/api/pembayaran/${params.id}`),
          fetch(`/api/pembayaran/${params.id}/evidence?page=1&limit=9`),
        ]);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch(
        `/api/pembayaran/${params.id}/evidence/${deleteTargetId}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleFileChange = async (
    ...
    try {
      ...
      const response = await fetch(
        `/api/pembayaran/${params.id}/evidence/${isEditing}`,
        {
          method: "PUT",
          body: formData,
        }
  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      const response = await fetch(`/api/pembayaran/${params.id}/evidence`, {
        method: "POST",
        body: formData,
      });
              ...
              <div key={item.id} className="group">
                <div className="overflow-hidden bg-white rounded-lg shadow-md">
                  <div className="relative">
                    <img
                      src={`/api/pembayaran/${params.id}/evidence/${item.id}`}
                      ...
                      onClick={() => {
                        setSelectedImage(
                          `/api/pembayaran/${params.id}/evidence/${item.id}`
                        );
