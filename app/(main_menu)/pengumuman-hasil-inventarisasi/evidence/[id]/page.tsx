...
export default function EvidencePage() {
  ...
  const fetchEvidenceData = async (pageNum: number) => {
    try {
      const response = await fetch(
        `/api/pengumuman/${params.id}/evidence?page=${pageNum}&limit=9`
      );
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        const pengumumanResponse = await fetch(`/api/pengumuman/${params.id}`);
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await fetch(`/api/pengumuman/${params.id}/evidence`, {
        method: "POST",
        body: formData,
      });
  ...
  const handleDeleteConfirm = async () => {
    if (!deleteTargetId || !params.id) return;

    try {
      const response = await fetch(
        `/api/pengumuman/${params.id}/evidence/${deleteTargetId}`,
        { method: "DELETE" }
      );
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/pengumuman/${params.id}/evidence/${selectedEditId}`,
        {
          method: "PUT",
          body: formData,
        }
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-6">
        <Link href="/pengumuman-hasil-inventarisasi">
              ...
              <div key={item.id} className="group">
                <div className="overflow-hidden bg-white rounded-lg shadow-md">
                  <div className="relative">
                    <img
                      src={`/api/pengumuman/${params.id}/evidence/${item.id}`}
                      ...
                      onClick={() => {
                        setSelectedImage(
                          `/api/pengumuman/${params.id}/evidence/${item.id}`
                        );
