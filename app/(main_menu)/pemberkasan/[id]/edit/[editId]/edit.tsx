...
export default function EditPemberkasan({ session }: { session: any }) {
  const { id, editId } = useParams();
  const router = useRouter();
  const { showAlert } = useAlert();
  const [loading, setLoading] = useState(true);
  const [submitting, setSubmitting] = useState(false);
  const [data, setData] = useState<PemberkasanData | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`/api/pemberkasan/${editId}`);
        if (!response.ok) throw new Error("Failed to fetch data");

        const pemberkasanData = await response.json();
        setData(pemberkasanData);
        setFormData({
          tanggalPelaksanaan: pemberkasanData.tanggalPelaksanaan.split("T")[0],
          keterangan: pemberkasanData.keterangan,
        });
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const kartuKeluargaInput = document.getElementById(
        "kartukeluarga"
      ) as HTMLInputElement;
      const alasHakInput = document.getElementById(
        "alashak"
      ) as HTMLInputElement;

      if (
        ktpInput?.files?.[0] ||
        kartuKeluargaInput?.files?.[0] ||
        alasHakInput?.files?.[0]
      ) {
        const fileFormData = new FormData();

        if (ktpInput?.files?.[0]) {
          fileFormData.append("ktp", ktpInput.files[0]);
        }

        if (kartuKeluargaInput?.files?.[0]) {
          fileFormData.append("kartukeluarga", kartuKeluargaInput.files[0]);
        }

        if (alasHakInput?.files?.[0]) {
          fileFormData.append("alashak", alasHakInput.files[0]);
        }

        const fileResponse = await fetch(
          `/api/pemberkasan/${editId}/update-file`,
          {
            method: "PUT",
            body: fileFormData,
          }
