...
const TabelPemberkasan = ({ session }: { session: any }) => {
  const [pemberkasanData, setPemberkasanData] = useState<PemberkasanData[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`/api/pemberkasan?itemId=${id}`, {
          cache: "no-store",
        });
        const data = await response.json();
        console.log("Data Pembayaran dari API:", data); // Debugging
        setPemberkasanData(data);
      } catch (error) {
  ...
  const totalPages = Math.max(1, Math.ceil(pemberkasanData.length / itemsPerPage));

  const handleFileView = async (id: string, type: string) => {
    try {
      const response = await fetch(
        `/api/pemberkasan/${id}/formulir?type=${type}`
      );
      if (!response.ok) {
  ...
  const confirmDelete = async () => {
    if (deleteId) {
      try {
        const response = await fetch(`/api/pemberkasan/${deleteId}`, {
          method: "DELETE",
        });

        if (response.ok) {
          setPemberkasanData((prevData) =>
  ...
  const exportToExcel = () => {
    ...
    const processedData = pemberkasanData.map((item, index) => ({
      "NO.": index + 1,
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Data Pemberkasan");

    XLSX.writeFile(workbook, "Data Pemberkasan.xlsx");
  };
        ...
        <div className="overflow-x-auto bg-transparent border-2 border-gray-400 rounded-md">
          <table className="min-w-full divide-y-2 divide-gray-400">
            ...
            <tbody className="divide-y-2 divide-gray-400">
              {pemberkasanData.map((item, index) => (
                <tr
                  key={item.id}
                ...
                >
                  ...
                  <td className="px-6 py-4 text-center whitespace-nowrap">
                    {item.ktp ? (
                      <button
                        onClick={() => handleFileView(item.id, "ktp")}
                  ...
                  <td className="px-6 py-4 text-center whitespace-nowrap">
                    {item.kartukeluarga ? (
                      <button
                        onClick={() => handleFileView(item.id, "kartukeluarga")}
                        className="px-4 py-2 text-white transition duration-200 ease-in-out rounded-lg bg-color3 hover:bg-color8"
                      >
                        <div className="flex items-center space-x-3 text-sm font-semibold uppercase">
                          <FaFileImage className="text-xl" />
                  ...
                  <td className="px-6 py-4 text-center whitespace-nowrap">
                    {item.alashak ? (
                      <button
                        onClick={() => handleFileView(item.id, "alashak")}
                        className="px-4 py-2 text-white transition duration-200 ease-in-out rounded-lg bg-color3 hover:bg-color8"
                      >
                        <div className="flex items-center space-x-3 text-sm font-semibold uppercase">
                          <FaFileImage className="text-xl" />
                  ...
                  {session.user.role === "admin" ? (
                    <td className="px-6 py-4 text-center whitespace-nowrap">
                      <div className="flex justify-center space-x-3">
                        <button
                          onClick={() =>
                            router.push(`/pemberkasan/${id}/edit/${item.id}`)
                          }
