...
import ExportButtonPenebangan from "@/app/_components/export/ExportButtonPenebangan";
...
interface EvidencePenebangan {
  id: string;
  file: string;
}

const TabelPenebangan = ({ session }: { session: any }) => {
  const [penebanganData, setPenebanganData] = useState<PenebanganData[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`/api/penebangan?itemId=${id}`, {
          cache: "no-store",
        });
        const data = await response.json();
        console.log("Data Pembayaran dari API:", data); // Debugging
        setPenebanganData(data);
      } catch (error) {
  ...
  const confirmDelete = async () => {
    if (deleteId) {
      try {
        const response = await fetch(`/api/penebangan/${deleteId}`, {
          method: "DELETE",
        });

        if (response.ok) {
          setPenebanganData((prevData) =>
  ...
  const exportToExcel = () => {
    ...
    const processedData = penebanganData.map((item, index) => ({
      "NO.": index + 1,
    ...
    <div className="px-6 pt-32 pb-20">
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-gray-800">Tabel Penebangan</h1>
          {session.user.role === "admin" ? (
            <div className="flex space-x-4">
              <ExportButtonPenebangan penebanganData={penebanganData} />
              <Link
                href={`/penebangan/${id}/form`}
            ...
            <tbody className="divide-y-2 divide-gray-400">
              {penebanganData.map((item, index) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 text-center whitespace-nowrap">
                    <Link href={`/penebangan/${id}/evidence/${item.id}`}>
                      <button
                  ...
                  {session.user.role === "admin" ? (
                    <td className="px-6 py-4 text-center whitespace-nowrap">
                      <div className="flex justify-center space-x-3">
                        <button
                          onClick={() =>
                            router.push(`/penebangan/${id}/edit/${item.id}`)
                          }
...
export default TabelPenebangan;
