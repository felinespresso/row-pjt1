...
export default function EvidencePage() {
  ...
  const fetchEvidenceData = async (pageNum: number) => {
    try {
      const response = await fetch(
        `/api/penebangan/${params.id}/evidence?page=${pageNum}&limit=9`
      );
  ...
  useEffect(() => {
    console.log("Fetching evidence for ID:", params.id); // Debugging ID

    const fetchInitialData = async () => {
      try {
        const [penebanganResponse, evidenceResponse] = await Promise.all([
          fetch(`/api/penebangan/${params.id}`),
          fetch(`/api/penebangan/${params.id}/evidence?page=1&limit=9`),
        ]);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch(
        `/api/penebangan/${params.id}/evidence/${deleteTargetId}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleFileChange = async (
    ...
    try {
      ...
      const response = await fetch(
        `/api/penebangan/${params.id}/evidence/${isEditing}`,
        {
          method: "PUT",
          body: formData,
        }
  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      const response = await fetch(`/api/penebangan/${params.id}/evidence`, {
        method: "POST",
        body: formData,
      });
              ...
              <div key={item.id} className="group">
                <div className="overflow-hidden bg-white rounded-lg shadow-md">
                  <div className="relative">
                    <img
                      src={`/api/penebangan/${params.id}/evidence/${item.id}`}
                      ...
                      onClick={() => {
                        setSelectedImage(
                          `/api/penebangan/${params.id}/evidence/${item.id}`
                        );
