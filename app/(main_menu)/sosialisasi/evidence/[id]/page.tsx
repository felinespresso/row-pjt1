...
export default function EvidencePage() {
  ...
  const fetchEvidenceData = async (pageNum: number) => {
    try {
      const response = await fetch(
        `/api/sosialisasi/${params.id}/evidence?page=${pageNum}&limit=9`
      );
    ...
    const fetchInitialData = async () => {
      try {
        const [sosialisasiResponse, evidenceResponse] = await Promise.all([
          fetch(`/api/sosialisasi/${params.id}`),
          fetch(`/api/sosialisasi/${params.id}/evidence?page=1&limit=9`),
        ]);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch(
        `/api/sosialisasi/${params.id}/evidence/${deleteTargetId}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleFileChange = async (
    ...
    try {
      ...
      const response = await fetch(
        `/api/sosialisasi/${params.id}/evidence/${isEditing}`,
        {
          method: "PUT",
          body: formData,
        }
  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      const response = await fetch(`/api/sosialisasi/${params.id}/evidence`, {
        method: "POST",
        body: formData,
      });
    ...
    >
      ...
      <div className="p-6 bg-white rounded-lg shadow-lg">
        <div className="mb-6">
          ...
          <h2 className="text-sm font-semibold text-color3">{data.spanTower}</h2>
        </div>
                ...
                <div className="overflow-hidden bg-white rounded-lg shadow-md">
                  <div className="relative">
                    <img
                      ...
                      onClick={() => {
                        setSelectedImage(
                          `/api/sosialisasi/${params.id}/evidence/${item.id}`
                        );
