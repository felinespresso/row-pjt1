...
export default function EditSosialisasi({ session }: { session: any }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`/api/sosialisasi/${editId}`);
        if (!response.ok) throw new Error("Failed to fetch data");

        const sosialisasiData = await response.json();
        setData(sosialisasiData);
        setFormData({
          tanggalPelaksanaan: sosialisasiData.tanggalPelaksanaan.split("T")[0],
          keterangan: sosialisasiData.keterangan,
        });
      } catch (error) {
    ...
    try {
      ...
      if (beritaAcaraInput?.files?.[0] || daftarHadirInput?.files?.[0]) {
        ...
        const fileResponse = await fetch(
          `/api/sosialisasi/${editId}/update-file`,
          {
            method: "PUT",
            body: fileFormData,
          }
